...
const FundRiserForm = () => {
    ...
    const riseFund = (e) =>{
       ...
       const onSuccess = () =>{
        ...
        setDeadline("")
        toastSuccess("Fund raiser created");
      }
  ...
  return (
    <>
        <h1 className="font-sans font-bold text-xl">Start a new fundraiser</h1>
        <form onSubmit={(e)=>riseFund(e)}>
            ...
            <button className="p-2 w-full bg-[#F56D91] text-white rounded-md hover:bg-[#d15677]" disabled={btnLoading} >{btnLoading?"Loading...":"Submit"}</button>
        </form>
    </>
  )
}
